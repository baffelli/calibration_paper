...
def tcr_recap(inputs, outputs, threads, config, params, wildcards):
    ...
    C = mat.coherencyMatrix(C_root, inputs['C_par'], gamma=True, bistatic=True, basis='lexicographic')
    ...
    with open(outputs[0], 'w+') as of:
        ...
        tabwrite.writerow(
            ['name', 'slant range', 'HH-VV amplitude imbalance', 'HH-VV phase imbalance', 'Polarization purity', 'RCS',
        ...
        for ref in refl_list:
            ...
            sw = params.sw
            aw = params.aw
            ptarg_zoom_C, rplot_C, azplot_C, mx_pos, resolution_dict, azvec, rvec = cf.ptarg(C[:, :, :, :],
                                                                                             float(idx_r),
                                                                                             float(idx_az), azwin=10,
                                                                                             rwin=10, sw=tuple(
                    config['calibration']['search_window']))
