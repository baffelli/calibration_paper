...
\begin{figure*}[ht]
	...
	These plots represent the time domain envelope of a point target (represented by a trihedral reflector in (a)) as a function of chirp time $t$ (y-axis) and slow time $\tau = n t_{chirp}$ (x-axis). The y-axis is proportional to the transmitted frequency, while the x-axis is proportional to the rotation angle of the antenna. If the antennas mainlobe would not change is direction during the chirp, a beat signal as shown in the bottom half of (b) would be obtained: its frequency is proportional to the distance to the target and it covers the slow time extent where the object is within the antenna beamwidth. When the antenna displays a frequency-squint behavior, the energy of the target is skewed because the target is only illuminated at the moment when the mechanical and the electrical antenna pointing direction match, causing its response to occupy several azimuth cells. After range compression, that entails a Fourier transform along the $t$ axis, this effect would cause a loss of range resolution because only a fraction of the transmitted bandwidth would be used.}
...
\begin{table}[ht]
	...
	\caption{Summary of the employed TCRs: Distance from the radar, expected radar crosssection, type of reflector.}
